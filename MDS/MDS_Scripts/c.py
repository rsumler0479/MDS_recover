...
def get_current():
    ...
    for each in reporting:
        if 'Reported By' in each.text:
            time.sleep(1.5)
            reported_by.append(each.text.strip().strip('Reported By:').title())
...
driver = webdriver.Chrome(
    executable_path='/Users/rashidsumler/Desktop/MDS_Scripts/chromedriver')
...
try:
    ...
    with open(add_extension, 'w', newline='') as f:
        ...
        fieldnames = ['Owner Name', 'Owner Address', 'Reported By',
                      'Type of Account', 'Amount', 'CoOwners', 'Securities', 'Property ID']
